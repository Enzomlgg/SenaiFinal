...
namespace BarraByTech.Areas.Identity.Pages.Account.Enderecos
{
    public class EditModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            // Mantém o UserId original
            Endereco.UserId = usuarioId;

            var atualizado = await _clienteService.UpdateEnderecoAsync(Endereco);

            if (!atualizado)
            {
                ModelState.AddModelError("", "Não foi possível atualizar o endereço.");
                return Page();
            }
